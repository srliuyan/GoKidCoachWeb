...
- 不修改 `ContextFusion`
- shallow tactical verification 仅作为有界安全层激活，不作为通用搜索或强度调参
- 不加载 `evaluation/*.json`
...
## V1.5.1 Cleanup Notes

活跃运行时版本信息只来自 `build-info.js`。`app.js`、`product-support.js`、`sw.js`、SGF 导出、debug 导出、存档和 UI 版本显示都必须读取同一个 BUILD_INFO 对象。

活跃难度映射由发布层和 `DifficultyController` 共同消费：入门 720、基础 840、进阶 980、自适应起点 880。清理阶段不改变这些数值，也不改变候选排序、评分权重、ContextFusion、局部阅读深度或最终选择策略。

评估框架继续保留在 `evaluation/`，但不进入浏览器运行时缓存。`run-cleanup-audit.js` 生成依赖图、service-worker 资产审计、未使用代码审计和行为锁比较，用来证明清理没有改变选点行为。
