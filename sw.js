...
const buildInfo = self.GoKidCoachBuildInfo;
const cacheName = buildInfo.serviceWorkerCache;
const assets = [
  "./",
  "./index.html",
  "./build-info.js",
...
self.addEventListener("activate", event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys.filter(key => key.startsWith("gokidcoach-web-") && key !== cacheName).map(key => caches.delete(key))
...
self.addEventListener("fetch", event => {
  ...
  if (url.pathname.includes("/evaluation/") || url.pathname.endsWith(".json") && url.pathname.includes("evaluation")) return;
